...
if df is not None:
    ...
    if not filtered_df.empty:
        with col3:
            ...
            selected_id = st.selectbox("관리번호(선택)", ["전체"] + list(existing_ids))
        ...
        with col4:
            ...
            # 선택된 ID 또는 입력된 ID 사용
            final_id = input_id if input_id else (selected_id if selected_id != "전체" else "")
        ...
        with col5:
            if '제조년도' in filtered_df.columns:
                ...
                year_ranges = ["전체"] + sorted(set(year_to_range(y) for y in years))
        ...
        if final_id:
            # 관리번호로 필터링
            equipment_df = filtered_df[filtered_df['관리번호'] == final_id]
        else:
            # 관리번호 없이 브랜드/모델만으로 필터링
            equipment_df = filtered_df
        
        if len(equipment_df) > 0:
            latest_record = equipment_df.sort_values('정비일자', ascending=False).iloc[0]
            ...
            with col2:
                st.write(f"**이전 정비일:** {latest_record.get('최근정비일자', pd.NaT) if pd.notna(latest_record.get('최근정비일자', pd.NaT)) else '정보 없음'}")
                st.write(f"**정비 내용:** {latest_record.get('정비내용', '정보 없음')}")
                st.write(f"**현장명:** {latest_record.get('현장명', '정보 없음')}")
                st.write(f"**관리번호:** {latest_record.get('관리번호', '정보 없음')}")
        else:
            st.info("선택한 조건에 맞는 정비 정보가 없습니다.")
        ...
        if st.button("고장 예측 실행"):
            with st.spinner("예측 분석 중..."):
                try:
                    ...
                    latest_data = equipment_df.sort_values('정비일자', ascending=False).iloc[0]
                    ...
                    avg_repair_days = equipment_df['AS처리일수'].mean()
