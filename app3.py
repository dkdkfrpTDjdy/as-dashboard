...
def extract_and_apply_region(df):
    if '현장' in df.columns:
        ...
        df['주소'] = [r[1] for r in results]
        
        # 주소가 아닌 값은 현장명으로 설정
        df['현장명'] = df['현장'].copy()
        mask = df['주소'].notna()
        df.loc[~mask, '현장명'] = df.loc[~mask, '현장']
    return df
...
# 정비일지와 수리비 데이터 병합 함수
def merge_repair_costs(df1, df3):
    if df1 is None or df3 is None:
        return df1
    
    try:
        # 데이터 복사
        maintenance_df = df1.copy()
        parts_df = df3.copy()
        
        # 데이터 형식 통일
        if '관리번호' in maintenance_df.columns:
            maintenance_df['관리번호'] = maintenance_df['관리번호'].astype(str)
        
        if '관리번호' in parts_df.columns:
            parts_df['관리번호'] = parts_df['관리번호'].astype(str)
        
        # 정비일자와 출고일자를 날짜형으로 변환
        if '정비일자' in maintenance_df.columns:
            maintenance_df['정비일자'] = pd.to_datetime(maintenance_df['정비일자'], errors='coerce')
        
        if '출고일자' in parts_df.columns:
            parts_df['출고일자'] = pd.to_datetime(parts_df['출고일자'], errors='coerce')
        
        # 병합 결과를 저장할 리스트
        merged_data = []
        
        # 정비일지의 각 행에 대해
        for _, maint_row in maintenance_df.iterrows():
            maint_id = maint_row['관리번호']
            maint_date = maint_row['정비일자']
            maint_person = str(maint_row.get('정비자번호', ''))
            
            # 해당 관리번호, 정비자, 날짜(±30일) 조건에 맞는 부품 데이터 필터링
            matching_parts = parts_df[
                (parts_df['관리번호'] == maint_id) & 
                (parts_df['출고자'].astype(str) == maint_person) &
                (abs((parts_df['출고일자'] - maint_date).dt.days) <= 30)
            ]
            ...
            if not matching_parts.empty:
                ...
                # 원본 행 복사 후 수리비 정보 추가
                maint_row_copy = maint_row.copy()
                maint_row_copy['수리비'] = total_cost
                maint_row_copy['사용부품'] = parts_list
                
                merged_data.append(maint_row_copy)
            else:
                # 매칭이 없으면 원본 행 유지하고 수리비와 부품은 NaN으로 설정
                maint_row['수리비'] = np.nan
                maint_row['사용부품'] = np.nan
                merged_data.append(maint_row)
        
        # 결과 DataFrame 생성
        result_df = pd.DataFrame(merged_data)
        
        # 수리비 컬럼이 없는 경우 0으로 채움
        if '수리비' not in result_df.columns:
            result_df['수리비'] = 0
        else:
            result_df['수리비'] = result_df['수리비'].fillna(0)
            
        # 사용부품 컬럼이 없는 경우 빈 문자열로 채움
        if '사용부품' not in result_df.columns:
            result_df['사용부품'] = ""
        else:
            result_df['사용부품'] = result_df['사용부품'].fillna("")
        
        return result_df
        
    except Exception as e:
        st.error(f"정비일지와 수리비 데이터 병합 중 오류 발생: {e}")
        import traceback
...
def display_integrated_dashboard(df, category_name):
    ...
    # 1. 월별 AS건수 + 월별 평균 가동 및 수리시간 + 월별 평균 수리비
    ...
    with col3:
        # 월별 평균 수리비 그래프 (수리비 컬럼이 있는 경우)
        if '수리비' in df.columns and '정비일자' in df.columns:
    ...
    # 수리비 관련 추가 분석 (데이터가 있는 경우)
    if '수리비' in df.columns:
        st.header("수리비 상세 분석")
        
        # 수리비가 많이 사용된 현장 Top 15
        ...
        with col2:
            ...
            if '정비자번호' in df.columns and '정비자' in df.columns:
                # 정비자번호와 이름 조합
                df['정비자정보'] = df['정비자'].astype(str) + " (" + df['정비자번호'].astype(str) + ")"
                worker_costs = df.groupby('정비자정보')['수리비'].sum().sort_values(ascending=False).head(15)
...
if df1 is not None or df3 is not None:
    ...
    if df1 is not None:
        menu_options.append("대시보드")  # 정비일지 대시보드를 대시보드로 변경
        menu_options.extend(["고장 유형 분석", "브랜드/모델 분석", "고장 예측"])

    # 원하는 순서대로 메뉴를 정렬
    sorted_menu_options = []
    desired_order = ["대시보드", "고장 유형 분석", "브랜드/모델 분석", "고장 예측"]  # 정비내용 분석 제거
    ...
    4. **고장 예측**: 기계학습 모델을 활용한 재정비 기간 및 증상 예측
